...
class Ant:
    ...
    def move_with_acceleration(self, vector):
        ...
        if random.choice([True, False, False]):
            x_entropy = random.randint(-1, 1)
            y_entropy = random.randint(-1, 1)
...
def follow_mouse(ant, distance_to_mouse, distance, reverse, enable_physics, enable_weird, factor, attract):
    ...
    if enable_physics:
        ...
        if enable_weird and distance >= weird_radius:
            new_v2 = mv(distance_to_mouse, (-1)) # put -1
            ...
            # ant.update_velocity(mv(move, 3)) bu da güzel
            ant.tp(mv(move, 3)) # bunu yapınca biraz titriyolar
            # ant.move(move)
        if attract:
...
def modify_ants_and_simulate(WIN, pause, reverse, enable_physics, enable_vel_color, enable_weird, attract):
    WIN.fill(BACKGROUND)
    ...
    for ant in ants:
        ...
        if not pause:
            ...
            follow_mouse(ant, distance_to_mouse, distance, reverse, enable_physics, enable_weird, factor, attract)
...
def main():
    ...
    while run:
        ...
        modify_ants_and_simulate(WIN, pause, reverse, enable_physics, enable_vel_color, enable_weird, attract)
    pygame.quit()
