...
# gravity ayarlarıyla oynadım diye garip şeyler oluyor
...
class Ant:
    ...
    def move_with_acceleration(self):
        ...
        head.x += self.vel_vector[0] + randomness[0]
        head.y += self.vel_vector[1] + randomness[1]
...
def follow_mouse(ant, distance_to_mouse, distance, reverse, enable_physics, factor):
    coeff = 1
    if reverse:
        coeff = -1
    ...
    if enable_physics:
        ant.update_velocity(mv(move, factor))
        ant.move_with_acceleration()
...
def color4proximity(ant, factor, distance):
...
def draw_window(WIN, ants, color, background, enable_prox_color, distance):
    """WIN.fill(background)
...
def modify_ants_and_simulate(WIN, pause, reverse, enable_physics, enable_prox_color):
    WIN.fill(BACKGROUND)
    if not enable_prox_color:
        manage_colors()
    ...
    for ant in ants:
        ...
        if enable_physics or enable_prox_color:
        ...
        if not pause:
            if distance >= 100:
                ...
                print(len(temp), len(ant.vel_vector))
                ant.vel_vector = av(ant.vel_vector, temp)
            follow_mouse(ant, distance_to_mouse, distance, reverse, enable_physics, factor)
        ...
        if enable_prox_color:
            if enable_physics:
                unique_color = color4proximity(ant, factor, distance)
                for part in ant.body:
                    pygame.draw.rect(WIN, (unique_color[0], unique_color[1], unique_color[2]), part)
            else:
                for part in ant.body:
...
def main():
    global ant_color, BACKGROUND, ant_color_change_vector
    ...
    # depending on their proximity to the mouse
    enable_prox_color = False
    while run:
        clock.tick(FPS)
        ...
        for event in pygame.event.get():
            ...
            # most keyboard controls
                ...
                if event.key == pygame.K_p:
                    ...
                    enable_prox_color = not enable_prox_color

        keys_pressed = pygame.key.get_pressed()

        modify_ants_and_simulate(WIN, pause, reverse, enable_physics, enable_prox_color)
    pygame.quit()
